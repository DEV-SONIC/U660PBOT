...
async def _(event):
    ...
    jmevent = await edit_or_reply(event, "` ⌔︙جـار انشـاء رابـط تلجراف`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f" ⌔︙انـشاء حسـاب تلجراف جـديد {auth_url} لـهذه الجلـسة. \n**لا تعـطي هـذا الرابـط لاي احـد, حـتى لو قـالو انـهم مـن شـركة التلجرام!**",
        )
    optional_title = event.pattern_match.group(5)
    if not event.reply_to_msg_id:
        return await jmevent.edit(
            "` ⌔︙قـم بالـرد عـلى هـذه الرسـالة للحـصول عـلى رابـط تلجراف فـورا`",
        )
