...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"- حذࢪتك وقلتلك لا تكࢪࢪ تَم حظࢪك بنجاح مقدرش تزعج المالك \n- - باي 🙁🤍"
...
لديك {warns}/{totalwarns} من التحذيرات لا تقوم بتكرار رسائلك حتى لا يقوم البوت بحظرك.

• يمكنك اختيار ماذا تريد من الاسفل 🤍"""
    else:
        USER_BOT_NO_WARN = f"""**- اهلا بك {mention} \n صاحب الحساب غير متواجد حاليا الرجاء ترك ما تريده برسالة واحدة فقط
الرجاء عدم تكرار الرسائل لانه سيتم حظرك **
لديك {warns}/{totalwarns}  من التحذيرات لا تقوم بتكرار رسائلك حتي  لا يقوم البوت بحظرك.
...
تعتقد هخليك تكرر رسائلك لا اسف هحظرك من الحساب. \
حاليا متقدرش  تتكلم الى ان ياتي صاحب الحساب ويقوم بالغاء الحظر"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """ لازم تصبر مالك الحساب مشافش الرسالة انتظر. \
مالك الحساب يرد على الكل بس ما اعرف اذا كان في كام شخص يتجاهلهم بس اصبر
مالك الحساب راح يرد عليك لما يكون متصل , اذا رد عليك اصلا
...
async def do_pm_request_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """ لازم تصبر مالك الحساب ما شاف الرسالة انتظر. \
مالك الحساب يرد على الكل بس ما اعرف اذا كان في كام شخص يتجاهلهم بس اصبر
...
**اتمنى ما تكرر الرسائل حتى ما اضطر احظرك 🙂🌿**"""
    ...
    USER_BOT_WARN_ZERO = f"**اتذكر حذرتك وقلت لك لا تكرر اكثر من خيار ولا ترسل رسائل مجرد انتظر. \
تعتقد راح اخليك تكرر لا اسف راح احظرك من الحساب. \
حاليا متقدرش بعد تتكلم الى ان ياتي صاحب الحساب ويقوم بالغاء الحظر. 🙂💘**"
    await event.reply(USER_BOT_WARN_ZERO)
    await event.client(functions.contacts.BlockRequest(chat.id))
    the_message = f"#حـماية الـخاص\
                \n[{get_display_name(chat)}](tg://user?id={chat.id}) تـم حـظره\
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق سونيك
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def pmpermit_on(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق سونيك
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائـمة الحـماية سونيك.txt",
