# @U660P - < https://t.me/U660P >
# Copyright (C) 2021 - U660P-AR
# All rights reserved.
#
# This file is a part of < https://github.com/DEV-SONIC/u660p >
# Please read the GNU Affero General Public License in;
# < https://github.com/DEV-SONIC/u660p/blob/master/LICENSE
# ===============================================================
import re
...
from userbot import Config, Sonic
...
@Sonic.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await Sonic.tgbot.send_message(
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await Sonic.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await Sonic.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await Sonic.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await Sonic.tgbot.get_entity(BOTLOG_CHATID)
            await Sonic.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await Sonic.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@Sonic.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await Sonic.get_entity(user_id)
    except Exception as e:
        ...
        await c_q.edit(
            f"▾∮ تم حظر المستخدم بسبب التكرار❗️ ↶**\n**▾∮الاسم ⪼ **`{user_name}`\n**▾∮الايدي ⪼ **`{user_id}`\n**▾∮الرابط ⪼** 「{_format.mentionuser(user_name , user_id)}**⍣ⵧⵧⵧⵧⵧسونيكⵧⵧⵧⵧⵧ⍣**\n[سورس سونيك 🧸♥](https://t.me/U660P)"
        )
...
@Sonic.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@Sonic.bot_cmd(incoming=True, func=lambda e: e.is_private)
@Sonic.bot_cmd(edited=True, func=lambda e: e.is_private)
